...
include '../static/db/conection.php';
	...
	$sql =mysqli_query($link,"UPDATE usuarios SET nombre='$_POST[nombre]',apellido='$_POST[apellido]',dni='$_POST[dni]',email='$_POST[email]',localidad='$_POST[localidad]',telefono='$_POST[telefono]' WHERE dni=$_SESSION[id]");
	}


	//Opcion modificacion de cuenta
	if($_POST['account']=='true'){
	$sql1 =mysqli_query($link,"UPDATE usuarios SET usuario='$_POST[usuario]' WHERE dni=$_SESSION[id]");

    $clave1 = $_POST['password1'];
    $clave2 = $_POST['password2'];

    if (($clave1 == $clave2) and ($clave1 and $clave2 != '')){
	$sql2=mysqli_query($link,"UPDATE usuarios SET password='$clave1' WHERE dni=$_SESSION[id]");
     }	}
    ...
    //Opcion de alta de imagen
     if($_POST['newimage']=='true'){
		...
		$sql=mysqli_query($link,"UPDATE usuarios SET imagen= 'user/$target_path' WHERE dni=$_SESSION[id]"); 
	}
